...
let alasqlp = require('alasqlp');
let path = require('path');
let fs = require('fs');
let stdin = process.openStdin();
let yargs = require('yargs')
	.strict()
	.usage('AlaSQL command-line utility (version ' + alasqlp.version + ')\n\nUsage: $0 [options] [sql] [params]')
...
.version('v', 'Echo AlaSQL version', alasqlp.version)
	.alias('v', 'version')
...
.epilog('\nMore information about the library: www.alasqlp.org')
...
function execute(sql, params) {
	...
	alasqlp.promise(sql, params)
		.then(function (res) {
			if (!alasqlp.options.stdout) {
